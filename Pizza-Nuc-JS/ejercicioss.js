/* Punto 2:
Renderizar una lista de películas con la data provista. Diseño a elección */
const movieElement = document.getElementById('container-movie');
...
const createElement = ({id,title,overview,release_date,vote_average,vote_count}) => {
   return `<div id="${id}"> 
        <h4>${title}</h4>
        <p>${overview}</p>
        <p>${release_date}</p>
        <p>Votos: ${vote_average}</p>
        <p>Contador de votos${vote_count}</p>
    </div>`
}
const mostartMovie =  movies.map(mov => createElement(mov)).join("");

movieElement.innerHTML = mostartMovie
